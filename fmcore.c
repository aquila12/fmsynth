...
int fmch_init(fmcontainer_t *ch /*, patch */) {
  ...
  ch->p_elements = calloc(n_elements, sizeof(fmel_t*));
  ch->n_elements = n_elements;

  if(ch->p_elements==0) return -1;

  /* Patch does this */
  fmel_t **el = ch->p_elements;
  fmosc_t* op = calloc(2, sizeof(fmosc_t));
...
void fmch_free(fmcontainer_t *ch) {
  /* TODO: Free each element */
  if(ch->p_elements) free(ch->p_elements);
